...
public class UserTaskSchedulerTests
{
    ...
    public UserTaskSchedulerTests()
    {
        ...
        _mockScoringStrategy
            .Setup(s => s.CalculateScore(It.IsAny<UnscheduledTask>()))
            .Returns(0);
    ...
    public void ScheduleTasks_WithEqualScores_SchedulesByDueDate()
    {
        ...
        var tasks = new List<UnscheduledTask>
        {
        ...
        Assert.Equal("Earlier Due Date", result.ScheduledTasks[0].OriginalTask.Name);
        Assert.Equal("Middle Due Date", result.ScheduledTasks[1].OriginalTask.Name);
        Assert.Equal("Later Due Date", result.ScheduledTasks[2].OriginalTask.Name);
    }
    ...
    public void ScheduleTasks_VerifyMockScoring_AllTasksHaveZeroScore()
    {
        ...
        _mockScoringStrategy.Verify(s => s.CalculateScore(It.IsAny<UnscheduledTask>()), Times.Once);
    }
    ...
    public void ScheduleTasks_WithSameDueDate_SchedulesByPriority()
    {
        ...
        Assert.Equal("High Priority Task", result.ScheduledTasks[0].OriginalTask.Name);
        Assert.Equal("Medium Priority Task", result.ScheduledTasks[1].OriginalTask.Name);
        Assert.Equal("Low Priority Task", result.ScheduledTasks[2].OriginalTask.Name);
        ...
        _mockScoringStrategy.Verify(
            s => s.CalculateScore(It.IsAny<UnscheduledTask>()), 
    ...
    private void SetupPriorityBasedScoring()
    {
        ...
        _mockScoringStrategy
            .Setup(s => s.CalculateScore(It.Is<UnscheduledTask>(t => 
    ...
    private UnscheduledTask CreateTestTask(
        int id,
        string name,
        DateTime dueDate,
        TimeSpan duration,
